...
void splitData(int iter, map<int, vector<Mat>> &data, map<int, Mat> &train, map<int, Mat> &test) {
	...
	for (auto i = data.begin(); i != data.end(); ++i){
		...
		trainset[i->first] = tmp;
	}

	for (auto i = trainset.begin(); i != trainset.end(); ++i){
		for (auto j = trainset.begin(); j != trainset.end(); ++j){
			if (i != j) {
				int idx = rand() % 20;
				i->second.push_back(data[j->first][idx]);
			}
		}		
	}

	for (auto i = trainset.begin(); i != trainset.end(); ++i){
		train[i->first] = asMatrix(i->second);
	}
}

void distanceNormalize(double *arr, int size){
	double min, max;
	Mat tmp(size, 1, CV_64F, &arr[0]);
	minMaxIdx(tmp, &min, &max);
	tmp = Scalar::all(max) - tmp;
	double f = max - min;
	tmp = tmp / f;
