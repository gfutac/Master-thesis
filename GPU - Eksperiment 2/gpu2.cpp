...
int main(){
	...
	load("D:\\Futac\\Diplomski rad\\Implementacija\\diplomski-rad-futac-goran\\dlanovi.txt", data, PALM);
	...
	for (int iter = 0; iter < 20; ++iter) {
		...
		if (iter == 0) {
			...
			projections = new float[rows * (vectors * vectors)];
		...
		for (auto c = train.begin(); c != train.end(); ++c){
			...
			a = p.projections.t();
			std::copy((float *)a.data, &(*((float *)a.data + vectors * vectors)), &projections[vectors * vectors * t]);
		...
		Mat projectionsMat(rows, vectors * vectors, CV_32F, projections);
