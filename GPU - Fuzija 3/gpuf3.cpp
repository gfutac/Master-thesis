...
int main(){
	cudaDeviceReset();
	colorText("GPU - Fuzija 3\n\n");

	// lica
	double pr1 = 0.985 / (0.985 + 0.9987);
	// dlanovi
	double pr2 = 1 - pr1;
	...
	for (int iter = 0; iter < 20; ++iter) {
		...
		if (iter == 0) {
			...
			fprojections = new float[rows * (vectors * templateSize)];
			fmeans = new float[rows * N];

			peigens = new float[rows * (N * vectors)];
			pprojections = new float[rows * (vectors * templateSize)];
		...
		for (auto c = trainFaces.begin(), d = trainPalms.begin(); c != trainFaces.end(), d != trainPalms.end(); ++c, ++d){
			loadBar(30, (float)bar++/(trainPalms.size() - 1));
			// lice
			int label = c->first;
			Mat tdata = c->second;
			...
			a = p.projections.t();
			std::copy((float *)a.data, &(*((float *)a.data + vectors * templateSize)), &fprojections[vectors * templateSize * t]);
			...
			// dlanovi
			tdata = d->second;
			pca pp(tdata, label, vectors, templateSize);

			a = pp.eigens.t();
			std::copy((float *)a.data, &(*((float *)a.data + N * vectors)), &peigens[N * vectors * t]);

			a = pp.projections.t();
			std::copy((float *)a.data, &(*((float *)a.data + vectors * templateSize)), &pprojections[vectors * templateSize * t]);
		...
		Mat fprojectionsMat(rows, vectors * templateSize, CV_32F, fprojections);
		...
		Mat pprojectionsMat(rows, vectors * templateSize, CV_32F, pprojections);
		...
		cout << "\tUtroseno vrijeme: " << elapsedTime << endl;
